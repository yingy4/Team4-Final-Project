...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    (nsA <- ns \\ "a";
     nsH <- nsA \ "@href";
     nH <- nsH.head) yield new URL(u, nH.toString())
  ...
  //con<- 将Future[FUture[...]]变为FUture[...]
    for (content <- getURLContent(u); con <- MonadOps.asFuture(getLinks(content))) yield con
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    //MonadOps.sequence----(xfs: Seq[Future[X]]): Seq[Future[Either[Throwable, X]]]
    //future------sequence[A, Seq[X] <: TraversableOnce[X]](in: Seq[Future[A]]): Future[Seq[A]]
    //wget(us2): Future[Seq[URL]]
